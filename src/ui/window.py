...
class Window(Adw.ApplicationWindow):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        super().__init__(title=app.name, *args, **kwargs)
        self._setup_size()
    ...
    def _setup_size(self) -> None:
        def on_close_request(window: Gtk.Window) -> None:
            ...
            self.get_application().settings.set_boolean("window-maximized", maximized)
    ...
    def _setup_file_store(self) -> None:
        def on_items_changed(
            file_store: FileStore, position: int, added: int, removed: int
        ) -> None:
            if len(file_store) == 0:
        ...
        def on_state_changed(file_store: FileStore, file_index: int) -> None:
            self.lookup_action("clean-metadata").set_enabled(
                not (
    ...
    def _setup_drop_target(self) -> None:
        def on_drop(widget: Gtk.DropTarget, value: Any, x: int, y: int):
    ...
    def _present_about(self) -> None:
        ...
        # Name only:    GeopJr
        # Name + URL:   GeopJr https://geopjr.dev
        # Name + Email: GeopJr <evan@geopjr.dev>
        ...
        about.add_other_app(
            ...
            "dev.geopjr.Archives",
            _("Archives"),
            _("Create and view web archives"),
        )
        ...
        about.add_other_app(
            ...
            _("Calligraphy"),
            _("Turn text into ASCII banners"),
        )
    ...
    def _setup_actions(self) -> None:
        ...
        def on_add_files(action: Gio.Action, parameters: None) -> None:
            self._file_chooser_dialog.open_multiple(
                self, None, self._on_file_chooser_dialog_response
            )
        ...
        def on_add_folders(action: Gio.Action, parameters: None) -> None:
            self._file_chooser_dialog.select_multiple_folders(
                self, None, self._on_folder_chooser_dialog_response
            )
        ...
        def on_remove_file(action: Gio.Action, parameters: GLib.Variant) -> None:
            self.file_store.remove_file_with_index(parameters.get_uint32())

        remove_file = Gio.SimpleAction.new("remove-file", GLib.VariantType.new("u"))
        ...
        def on_view_details(action: Gio.Action, parameters: GLib.Variant) -> None:
            f = self.file_store.get_file_with_index(parameters.get_uint32())
            self._details_view.view_file(f)
            self.open_details_view()

        view_details = Gio.SimpleAction.new("view-details", GLib.VariantType.new("u"))
        view_details.connect("activate", on_view_details)
        self.add_action(view_details)

        def on_close_details_view(action: Gio.Action, parameters: None) -> None:
            self.close_details_view()

        close_details_view = Gio.SimpleAction.new("close-details-view", None)
        ...
        def on_clean_metadata(action: Gio.Action, parameters: None) -> None:
            self.close_details_view()
            if not self.get_application().settings.get_boolean(
    ...
    def _on_sidebar_toggled(self, widget: Gtk.Widget, pspec: GObject.ParamSpec) -> None:
    ...
    def _on_folder_chooser_dialog_response(
        ...
        try:
            self.file_store.add_gfiles(
                file_dialog.select_multiple_folders_finish(result), True
            )
        except GLib.Error as e:
            if e.code == 2:
                # dismissed
    ...
    def _on_cleaning_warning_dialog_response(
        self, dialog: CleaningWarningDialog, response: str
    ) -> None:
        if response == "clean":
            self.file_store.clean_files()
    ...
    def _on_show_info_dialog(self) -> None:
        ...
        box.append(
            Gtk.Label(
                label=_(
                    "While Metadata Cleaner is doing its very best to display metadata, it doesn't mean that a file is clean from metadata if it doesn't show any. There is no reliable way to detect every single possible metadata for complex file formats. This is why you shouldn't rely on metadata's presence to decide if your file must be cleaned or not.\nMetadata Cleaner takes the content of the file and puts it into a new one without metadata, ensuring that any undetected metadata is stripped."
                ),
                xalign=0,
                wrap=True,
                wrap_mode=Pango.WrapMode.WORD_CHAR,
        ...
        box.append(
            Gtk.Label(
                label=_(
                    "Be aware that metadata is not the only way of marking a file. If the content itself discloses personal information or has been watermarked, traditionally or via steganography, Metadata Cleaner will not protect you."
                ),
                xalign=0,
                wrap=True,
                wrap_mode=Pango.WrapMode.WORD_CHAR,
        ...
        tbarview = Adw.ToolbarView(
            content=Gtk.ScrolledWindow(
                propagate_natural_height=True,
                propagate_natural_width=True,
                child=Adw.Clamp(
                    child=box, margin_start=6, margin_end=6, margin_bottom=6
                ),
            )
        )
